...
const bot = createBot(process.env.PHONE_NUMBER_ID, process.env.ACCESS_TOKEN);
...
class Resource {
  ...
  async sendWhatsappMessage(recipientId, txt, isTemplete) {
    console.log("req.body.isTemplete", recipientId, txt, isTemplete)
    let result = null;
    if (isTemplete) {
      result = await bot.sendTemplate(recipientId, txt, 'en_US')
    console.log("req.body.isTemplete", result)
    } else {
      result = await bot.sendText(recipientId, txt);
    }
  }
